...
# Get base URL from environment variable (set in Cloud Run)
...
BLOG_PROMPT_DOC_ID = os.environ.get("GOOGLE_DOC_BLOG_PROMPT_DOC_ID")
SHEET_ID = os.environ.get("GOOGLE_SHEET_ID")
OPENAI_API_KEY = os.environ.get("OPENAI_API_KEY")
FROM_EMAIL = os.environ.get("EMAIL_USERNAME")       
EMAIL_APP_PASSWORD = os.environ.get("EMAIL_APP_PASSWORD")
DESIGNER_EMAIL = os.environ.get("DESIGNER_EMAIL")
HEAD_EMAIL = os.environ.get("HEAD_EMAIL")
GAVARI_EMAIL = os.environ.get("GAVARI_EMAIL")
APPROVED_BLOGS_FOLDER_ID = os.environ.get("APPROVED_BLOGS_FOLDER_ID")
GIVEAWAY_DOC_ID = os.environ.get("GIVEAWAY_DOC_ID")
RESTRICTED_DOC_ID = os.environ.get("RESTRICTED_DOC_ID")
cta_agent = os.environ.get("CTA_SHEET_ID")
PRODUCT_SHEET_ID = os.environ.get("PRODUCT_SHEET_ID")
...
"""
        ...
        for _ in range(3):
            ...
            if len(blog_content) >= 9000 and len(blog_content.split()) >= 1500:
                ...
                logger.info(f"⚠️ Blog too short: {len(blog_content.split())} words, {len(blog_content)} characters. Retrying...")
...
giveaway_agent = GiveawayAgent(os.environ.get("GIVEAWAY_DOC_ID"))
...
cta_agent = CTAAgent(os.environ.get("CTA_SHEET_ID"))
...
# NEW: Endpoint to trigger topic generation (replaces background thread)
    ...
    try:
        ...
        if prompt_text:
            ...
            if sheet_agent.topic_exists(topic):
                return {"status": "skipped", "message": f"Duplicate topic: {topic}"}, 200

            success = sheet_agent.add_blog_entry(topic, keyword, matched_category, extra_keywords)
            if not success:
                return {"status": "failed", "message": "Failed to add blog entry"}, 500
            ...
            return {"status": "error", "message": "No prompt found in Google Doc"}, 404
    except Exception as e:
